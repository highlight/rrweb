...
| recordCanvas         | false              | Whether to record the canvas element. Available options:<br/>`false` (doesn't record),<br/>`true` (records every single mutation),<br/>`number` between 1 and 60 (records image snapshots in a web-worker a (maximum) number of times per second. Only supported where [`OffscreenCanvas`](http://mdn.io/offscreencanvas) is supported.) |
